...
def trade_routes(app):
    ...
    def sync_orders():
        ...
        if not fyers:
            return jsonify({"error": "FYERS not logged in"})
        return jsonify(fyers.orderbook())
    ...
    def get_ltp(symbol):
        ...
        if not fyers:
            return jsonify({"error": "FYERS not logged in"})
        return jsonify(fyers.quotes({"symbols": symbol}))
    ...
    def add_trade():
        ...
        cur.execute("SELECT capital, parts, live_master FROM settings WHERE id=1")
        s = cur.fetchone()

        capital, parts, live_master = s
        ...
        VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            mode, symbol, side, qty, entry, sl,
            round(target, 2), rr, "OPEN", 0, now, ""
        ))
